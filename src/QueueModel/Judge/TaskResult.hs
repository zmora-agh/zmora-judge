...
module QueueModel.Judge.TaskResult (TaskResult(..), result_id, compilation_log, tests_results) where
import Prelude ((+), (/))
...
data TaskResult = TaskResult{_result_id :: !(P'.Maybe P'.Int64), _compilation_log :: !(P'.Maybe P'.Utf8),
                             _tests_results :: !(P'.Seq Judge.TaskResult.TestResult)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''TaskResult

instance P'.Mergeable TaskResult where
  mergeAppend (TaskResult x'1 x'2 x'3) (TaskResult y'1 y'2 y'3)
   = TaskResult (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)

instance P'.Default TaskResult where
  defaultValue = TaskResult P'.defaultValue P'.defaultValue P'.defaultValue

instance P'.Wire TaskResult where
  wireSize ft' self'@(TaskResult x'1 x'2 x'3)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeOpt 1 3 x'1 + P'.wireSizeOpt 1 9 x'2 + P'.wireSizeRep 1 11 x'3)
  wirePut ft' self'@(TaskResult x'1 x'2 x'3)
   = case ft' of
       10 -> put'Fields
       11 -> do
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_compilation_log = Prelude'.Just new'Field}) (P'.wireGet 9)
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{_tests_results = P'.append (_tests_results old'Self) new'Field})
                    (P'.wireGet 11)
...
instance P'.TextMsg TaskResult where
  textPut msg
   = do
       ...
       P'.tellT "compilation_log" (_compilation_log msg)
       P'.tellT "tests_results" (_tests_results msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_result_id, parse'_compilation_log, parse'_tests_results]) P'.spaces
    ...
    where
        ...
        parse'_compilation_log
         = P'.try
            (do
               v <- P'.getT "compilation_log"
               Prelude'.return (\ o -> o{_compilation_log = v}))
