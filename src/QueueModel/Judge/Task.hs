...
module QueueModel.Judge.Task (Task(..), task_id, files, tests) where
import Prelude ((+), (/))
...
data Task = Task{_task_id :: !(P'.Maybe P'.Int64), _files :: !(P'.Seq Judge.Task.File), _tests :: !(P'.Seq Judge.Task.Test)}
          deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''Task

instance P'.Mergeable Task where
  mergeAppend (Task x'1 x'2 x'3) (Task y'1 y'2 y'3)
   = Task (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)

instance P'.Default Task where
  defaultValue = Task P'.defaultValue P'.defaultValue P'.defaultValue

instance P'.Wire Task where
  wireSize ft' self'@(Task x'1 x'2 x'3)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeOpt 1 3 x'1 + P'.wireSizeRep 1 11 x'2 + P'.wireSizeRep 1 11 x'3)
  wirePut ft' self'@(Task x'1 x'2 x'3)
   = case ft' of
       10 -> put'Fields
       11 -> do
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             18 -> Prelude'.fmap (\ !new'Field -> old'Self{_files = P'.append (_files old'Self) new'Field}) (P'.wireGet 11)
             26 -> Prelude'.fmap (\ !new'Field -> old'Self{_tests = P'.append (_tests old'Self) new'Field}) (P'.wireGet 11)
...
instance P'.TextMsg Task where
  ...
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_task_id, parse'_files, parse'_tests]) P'.spaces
