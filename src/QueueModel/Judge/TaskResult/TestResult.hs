...
module QueueModel.Judge.TaskResult.TestResult (TestResult(..), source_test_id, status, execution_time, ram_usage) where
import Prelude ((+), (/))
...
data TestResult = TestResult{_source_test_id :: !(P'.Maybe P'.Int64), _status :: !(P'.Maybe Judge.TaskResult.Status),
                             _execution_time :: !(P'.Maybe P'.Int64), _ram_usage :: !(P'.Maybe P'.Int64)}
                deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data, Prelude'.Generic)

Control.Lens.TH.makeLenses ''TestResult

instance P'.Mergeable TestResult where
  mergeAppend (TestResult x'1 x'2 x'3 x'4) (TestResult y'1 y'2 y'3 y'4)
...
instance P'.Default TestResult where
  defaultValue = TestResult P'.defaultValue P'.defaultValue P'.defaultValue P'.defaultValue

instance P'.Wire TestResult where
  wireSize ft' self'@(TestResult x'1 x'2 x'3 x'4)
   = case ft' of
       10 -> calc'Size
       11 -> P'.prependMessageSize calc'Size
       _ -> P'.wireSizeErr ft' self'
    where
        calc'Size = (P'.wireSizeOpt 1 3 x'1 + P'.wireSizeOpt 1 14 x'2 + P'.wireSizeOpt 1 3 x'3 + P'.wireSizeOpt 1 3 x'4)
  wirePut ft' self'@(TestResult x'1 x'2 x'3 x'4)
   = case ft' of
       10 -> put'Fields
       11 -> do
    ...
    where
        update'Self wire'Tag old'Self
         = case wire'Tag of
             ...
             24 -> Prelude'.fmap (\ !new'Field -> old'Self{_execution_time = Prelude'.Just new'Field}) (P'.wireGet 3)
             32 -> Prelude'.fmap (\ !new'Field -> old'Self{_ram_usage = Prelude'.Just new'Field}) (P'.wireGet 3)
...
instance P'.TextMsg TestResult where
  textPut msg
   = do
       ...
       P'.tellT "execution_time" (_execution_time msg)
       P'.tellT "ram_usage" (_ram_usage msg)
  textGet
   = do
       mods <- P'.sepEndBy (P'.choice [parse'_source_test_id, parse'_status, parse'_execution_time, parse'_ram_usage]) P'.spaces
    ...
    where
        ...
        parse'_execution_time
         = P'.try
            (do
               v <- P'.getT "execution_time"
               Prelude'.return (\ o -> o{_execution_time = v}))
        parse'_ram_usage
         = P'.try
            (do
               v <- P'.getT "ram_usage"
